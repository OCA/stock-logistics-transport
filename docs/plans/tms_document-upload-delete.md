...
1. Uploading documents from the driver/vehicle _Documents_ tab must **keep the user on
...
2. Add a per-row **Delete** action on the _Documents_ tab that **soft-deletes**
...
- `create_document_from_attachment` returns an `ir.actions.act_window` navigation
  action; the uploader widget calls `doAction(action)`, leaving the driver/vehicle form.
...
- **Model** — `create_document_from_attachment` stops returning a navigation action. It
  returns the created document ids instead:
  ```python
...
- **Widget** (`document_uploader.esm.js`) — in `onUploadComplete`, replace
  ...
  `document_ids` o2m recomputes and the new documents appear in the tab. The user stays
  on the driver/vehicle form.
...
- **Model** (`tms_document.py`):
  ...
  - Add a button method:
    ...
    `None` result into an `act_window_close` + `onClose` reload of the current form), so
    the row disappears while staying on the holder form.
...
- **Views** (`tms_driver_views.xml`, `fleet_vehicle_views.xml`) — render `document_ids`
  as an inline list with a per-row delete button:
  ```xml
...
- Update the three existing upload tests: assert the returned `ids` / `count` instead of
  the old action dict (`domain` / `view_mode` / `views`).
- New test: soft delete archives the document — `active` becomes `False`, it disappears
  from the holder's `document_ids`, and it no longer blocks `button_start_order`.
